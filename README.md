...
| Claude subagents | `extensions/agents/<name>.md` | Symlinked to `~/.claude/agents/`. `codex-runner` and `agy-runner` preload the matching delegation skill, run on `haiku` with no write tools, and return verification evidence rather than a verdict. Claude-only: OpenCode's agent schema differs, agy documents no definition path, Codex has no subagent primitive. |
