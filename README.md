...
| Managed settings and MCP config | `config/settings.json.tpl`, `config/opencode.json.tpl`, `config/cursor-mcp.json` | `setup.sh` renders Claude/OpenCode templates with absolute paths and copies Cursor MCP config. Codex owns `~/.codex/config.toml` and it is not managed. Use `@@CLAUDE_DIR@@`, `@@OPENCODE_DIR@@`, `@@DOTFILES_DIR@@` placeholders. |
| agy settings (scoped) | `config/agy-settings.json.tpl` | `scripts/sync-agy-settings.sh` deep-merges `permissions.allow`/`enableTelemetry` into `~/.gemini/antigravity-cli/settings.json` on every `setup.sh` run; `model`/`trustedWorkspaces` are agy's own runtime state and are left byte-identical. |
