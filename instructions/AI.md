...
The `agy` skill is the same delegation workflow against the Antigravity CLI (`agy`), for Gemini and the other model families in its catalog. It mirrors the `codex` skill's structure and rules, with four CLI-specific differences: `agy` has no working-directory flag, so each session is launched from its own worktree; the result contract is enforced mechanically with `--json-schema` rather than by asking; `--print-timeout` defaults to 5m and must be set from the expected task length; and a headless run whose tool calls were auto-denied still reports `"status": "SUCCESS"` with an empty response, so success is established from the diff and stderr, never from the status field. It deliberately does not use the `gemini` CLI, which a personal Google account cannot authenticate against (`IneligibleTierError`/`UNSUPPORTED_CLIENT`); that path needs enterprise or API-key auth.

Claude subagents live in `extensions/agents/<name>.md` and are symlinked by `setup.sh` into `~/.claude/agents/`. Two exist, `codex-runner` and `agy-runner`, and both are thin: each preloads the matching delegation skill with `skills:` so the skill stays the single cross-agent source of truth, and the subagent body only says what evidence to collect and how to report it. They run on `haiku` at `effort: low` with `tools: Bash, Read, Grep, Glob` - no Write or Edit - because their job is to gather evidence, not to judge the work or fix it. That split is what makes them worth having: the JSONL, diffs and test output stay in the subagent's context instead of the orchestrator's, and several dispatches can run at once.

Distribution is Claude-only, and deliberately so. OpenCode reads markdown agents from `~/.config/opencode/agents/` but its schema differs (a `prompt` field and provider-prefixed model names such as `ollama/qwen2.5-coder:14b`, against Claude's body and `haiku` alias), so one shared file would be broken for one of them; wiring OpenCode means a per-target transform like the one the command generator already does. `agy` exposes `--agent` and `agy agent` but ships no documented definition path. Codex has no subagent primitive at all - `codex agents` browses sessions, and its parallelism is separate `codex exec` processes in separate worktrees, which the `codex` skill already covers.

Two more Claude subagents, `advisor-quick` and `advisor-deep`, sit above `codex-runner`/`agy-runner` in this pipeline. Both are Opus-class and read-only (`Read, Grep, Glob, Bash`, no Write/Edit): they give a go/no-go opinion on a plan before it is dispatched, they never implement anything themselves. `effort` is static frontmatter, not something a single agent can flex per call, so complexity is handled by picking which file to invoke rather than parameterizing one: `advisor-quick` (`effort: low`) for a plan that is already fairly clear and just needs a second opinion; `advisor-deep` (`effort: high`) when the decision itself is the hard part - ambiguous scope, multiple viable approaches, or real risk if the wrong call is made.

Consulting an advisor is the orchestrator's discretion, not a rule that fires on every dispatch: reach for one before a non-trivial codex/agy dispatch (new worktree, multi-file, ambiguous scope, or anything the orchestrator is not confident about), and skip it for routine or mechanical dispatches where the plan is obvious. Skip it entirely, regardless of task shape, when the orchestrating session's own model is already Opus-class or above - consulting an equal-or-weaker advisor is pointless, the orchestrator's own judgment already covers what the advisor would add. When it is genuinely unclear whether the advisor call is worth the tokens, ask the user rather than guessing either way.

The reviewer tier needs no new subagent file - it is `codex-runner`/`agy-runner` dispatched with a stronger model/effort than the implementer used, so the same evidence-gathering shape applies to review as to implementation. Pick per task: `gpt-5.6-sol`/`medium` or `gpt-5.6-terra`/`high` via `codex-runner` for a Codex-side second look; an Opus-class Claude review (a plain `Task`/general-purpose dispatch, or `advisor-deep` if the question is really "should this be accepted" rather than "is this correct") when the review needs Claude's judgment specifically; or `agy-runner` with whichever model in its catalog is strong enough for the task - agy is fine for review as long as the model backing it is, not by default. A reviewer must differ in model or effort from whichever run produced the work, the same rule `references/review.md` already states for the `agy` skill.
