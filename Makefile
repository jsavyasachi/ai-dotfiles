test:
	...
	bash tests/sync-agy-settings.sh
...
lint:
	...
	bash -n tests/sync-agy-settings.sh
	...
	bash -n scripts/sync-agy-settings.sh
	python3 -m py_compile scripts/codex-models.py scripts/bands-validate.py
	if command -v shellcheck >/dev/null 2>&1; then shellcheck setup.sh tests/setup.sh tests/dispatch.sh tests/band-resolve.sh tests/bands-validate.sh tests/codex-models.sh tests/bands-drift.sh tests/sync-agy-settings.sh scripts/agy-dispatch.sh scripts/codex-dispatch.sh scripts/band-resolve.sh scripts/bands-drift.sh scripts/sync-agy-settings.sh; else echo "shellcheck not installed; skipping"; fi
